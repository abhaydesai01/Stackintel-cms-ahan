$(document).ready(function () {
	generateTableWithClickableRows();
});

function generateTableWithClickableRows() {
...
//							hiddenFields += "<br />";
							...
							//loop for generating the link starts here
							var linkUrl = "?ID=" + eachRowData.ID + "";
							for (var l = 0; l < tableData.columns.length; l++) {
								if (eachRowData[tableData.columns[l]] != null && eachRowData[tableData.columns[l]] != 'null' && eachRowData[tableData.columns[l]] != "NULL") {
									if (tableData.columns[l] != "ID" && tableData.columns[l] != "Id" && tableData.columns[l] != "id") {
										linkUrl += "&" + tableData.columns[l] + "=" + eachRowData[tableData.columns[l]] + "";
									}
								} else {
									linkUrl += "&" + tableData.columns[l] + "=null";
								}
							}
//							myTableData += "<tr id = "+eachRowData.ID+" title='Click Here' onclick='openLink(\""+tableData.miscellaneousData.href+linkUrl+"\");'>";
							myTableData += "<tr id = " + eachRowData.ID + " title='Click Here'>";
							
							//loop for generating the link ends here
							//loop for generating the table data starts here
							for (var l = 0; l < tableData.columns.length; l++) {
								if (eachRowData[tableData.columns[l]] != null && eachRowData[tableData.columns[l]] != 'null' && eachRowData[tableData.columns[l]] != "NULL") {
									if (tableData.columns[l] != "ID" && tableData.columns[l] != "Id" && tableData.columns[l] != "id") {
										myTableData += "<td style=\"white-space: nowrap\"><a href='" + tableData.miscellaneousData.href + linkUrl + "' target='_blank'>" + eachRowData[tableData.columns[l]] + "</a></td>";
									}
								} else {
									myTableData += "<td style=\"white-space: nowrap\">NA</td>";
								}
							}
							//loop for generating the table data Ends here
							myTableData += "</tr>";
						}
					} else {
						myTableData += "<tr>";
